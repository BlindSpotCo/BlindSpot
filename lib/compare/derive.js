...
export const DUTY_DEFAULTS = {
  ...
  // Gujarat stamp duty 4.9% + registration fee 1% (standard/base rate,
  // before the women-buyer concession some sources note) -- confirmed
  // across cleartax.in and propertysdeal.in.
  Ahmedabad:  { stampPct: 4.9, regPct: 1.0 },
  // West Bengal stamp duty is genuinely tiered -- 6% urban (Corporation/
  // Municipality area) vs. 5% rural (Panchayat area), plus an extra 1%
  // surcharge on the stamp duty portion for properties valued above
  // Rs1 crore -- confirmed across cleartax.in and the WB Directorate of
  // Registration & Stamp Revenue's own fee schedule. Kolkata itself is
  // entirely Corporation/Municipality area (KMC or Bidhannagar MC), so
  // the urban 6% base rate applies city-wide; the >Rs1cr surcharge is
  // NOT folded in here, matching every other city's single-flat-rate
  // convention in this table (none of them encode a price-tier
  // surcharge either).
  Kolkata:    { stampPct: 6.0, regPct: 1.0 },
