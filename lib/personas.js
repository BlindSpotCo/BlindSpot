...
//   2. defaultAreaWeight - unused now. Was the neighbourhood-vs-unit split
//                          in the combined BlindSpot score, back when a
//                          person could move that split themselves via a
//                          slider on the report. That slider is gone
//                          (ReportScreen.js hardcodes areaWeight at 0.5,
//                          see its own comment) and nothing reads this
//                          field any more -- left as-is per persona for
//                          whoever revives per-persona area/unit weighting,
//                          not wired into anything live today.
...
export const PERSONAS = {
  young_professional: {
    ...
    // Shown only for the active persona in PersonaPicker, right where it
    // was just picked -- Apeksha's UX review: choices need to say what
    // they actually do the moment you make them, not just carry a
    // one-line identity blurb. Plain-English version of avWeights/
    // defaultAreaWeight above, not new logic.
    effect: "Weights commute, connectivity, and the unit itself higher, schools count for very little. Starts the score 45% neighbourhood / 55% unit.",
    color: '#A5643B',
  ...
  family_buyer: {
    ...
    effect: "Weights safety and schools higher, roads and infrastructure count for less. Starts the score 60% neighbourhood / 40% unit.",
    color: '#632834',
    ...
    reportOverlay: {
      ...
      toneNote: 'Calm, complete, unhedged. Longer sentences are fine. Do not reassure. If crime, water or power sits in a weak band, state it inside the BlindSpot Verdict paragraph itself, not only later - this reader is the one a soft-pedalled warning actually harms.',
    },
  },
  investor: {
    id: 'investor',
    ...
    effect: "Weights infrastructure, power, and connectivity higher, since those drive rent and resale. Starts the score 55% neighbourhood / 45% unit.",
    color: '#77652E',
  ...
  broker: {
    ...
    effect: 'Uses the same neutral weighting AsliVastu itself uses, no lean toward any one buyer type. Starts the score 50/50.',
    color: '#3F411C',
