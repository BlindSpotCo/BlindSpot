...
export default function useMapCapture() {
  ...
  // So the page can say the map failed instead of 'loading' forever.
  ...
  const onScreenshot = useCallback((label, data, forIndex) => {
    ...
    if (done < SHOTS.length) {
      // A short breath so the iframe can settle between frames. It does not
      // need 350ms -- the settle-detection inside the iframe is what
      // actually decides when a frame is ready.
      nextShotRef.current = setTimeout(() => requestShot(done, run), 120);
      return;
    }
  ...
  // The explicit, in-app version of the unmount cleanup above -- for a
  // person who deliberately wants to stop a run that's still capturing
  // (picked the wrong floor or facing, say) without having to unmount
  // whatever owns this hook. Same exit, same 'capture-cancelled' message,
  // just triggered by a click instead of the component going away. A
  // no-op when nothing is in flight.
  const cancel = useCallback(() => {
    if (resolverRef.current || rejecterRef.current) finish(new Error('capture-cancelled'));
  }, [finish]);

  return { captureScreenshots, onReady, onScreenshot, onStatus, ready, failed, cancel };
}
