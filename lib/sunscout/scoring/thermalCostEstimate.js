...
// nothing to do with this unit's own sun exposure. Tuned against a
// metered field study of urban Indian households (Bath Univ., BSO 2022):
// single-AC homes averaged ~7.3 running hours/day across the whole
// summer (11kWh/day mean incl. zero-use days), with weekday/weekend
// ranges of 11-15h on days AC was actually run.
const BASELINE_HOURS_PER_NIGHT = 5;
...
// extra 5h here too; the two numbers can't drift out of sync.
const MAX_EXTRA_HOURS = 5;

// A typical 1.5-ton inverter split AC's average running draw, blended
// across the real ownership mix rather than assuming everyone bought a
// 5-star unit -- BEE's household survey found only 33% of owned units
// are 5-star (18% 4-star, 40% 3-star, which draws closer to 1.6-1.8kW).
const ASSUMED_AC_KW = 1.55;
...
// precision this estimate doesn't have. Widened from the initial ±15%:
// the same field study showed weekday-vs-weekend swings alone in that
// range, before any AC-count or efficiency variance.
...
export function estimateAcCost({ heatLoad, tariffPerKwh }) {
  ...
  return {
    ...
    methodology:
      ...
      `→ ~${Math.round(estRunHoursPerDay * 10) / 10}h/day for a standard 1.5-ton AC ` +
