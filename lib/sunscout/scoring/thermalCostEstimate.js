...
// Extra hours added at the worst-case heatLoad (matches shadeHeatScore's
// own CEILING_HEAT_LOAD=7, so a unit that scores 0 on Shade & Heat is the
// one that gets the full extra 4h here too -- the two numbers are meant
// to move together).
const MAX_EXTRA_HOURS = 4;
const HEAT_LOAD_CEILING = 7;
...
export function estimateAcCost({ heatLoad, tariffPerKwh }) {
  const heatRatio = clamp(heatLoad / HEAT_LOAD_CEILING, 0, 1);
  ...
  return {
    ...
    methodology:
      ...
      `scaled by this unit's own heat exposure (heatLoad=${heatLoad.toFixed(1)}/${HEAT_LOAD_CEILING}) ` +
