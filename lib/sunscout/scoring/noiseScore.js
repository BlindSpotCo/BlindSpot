...
// The report waits on this call before it can render at all -- even
// raced (primary + mirror at once, see below), a genuinely cold
// (never-queried) map patch can take several real seconds to hear back
// from Overpass's public server, and every one of those seconds is the
// user staring at "Reading the records for this address...". This caps
// how much of that wait the NOISE score specifically is allowed to add:
// past this, the report renders with a neutral noise score rather than
// keep the user waiting on a nice-to-have. The underlying fetch is NOT
// cancelled at that point -- see the after() call below -- it keeps
// running so the 60-day cache (CACHE_SECONDS) still gets populated,
// which is what makes the SECOND person to view this address (or this
// same person, refreshing) get the real score instantly instead of
// hitting this same soft cap forever.
const SOFT_DEADLINE_MS = 2200;
...
export async function fetchNearbyNoiseSources(lat, lon) {
  ...
  // only rejects (AggregateError) if every endpoint failed.
  const overpassPromise = Promise.any(OVERPASS_URLS.map((url) => queryOverpass(url, query))).catch(() => null);

  // Then THAT combined promise is raced against the soft deadline above.
  // A real answer inside the deadline is processed and returned as
  // normal; past the deadline, the report moves on without waiting, but
  // overpassPromise itself is neither cancelled nor abandoned -- after()
  // keeps the function alive to let it finish and write its result into
  // Next's fetch cache, purely for whoever asks about this map patch
  // next (including a plain refresh of this same page).
  const deadline = Symbol('noise-soft-deadline');
  const raced = await Promise.race([
    overpassPromise,
    new Promise((resolve) => setTimeout(() => resolve(deadline), SOFT_DEADLINE_MS)),
  ]);

  if (raced === deadline) {
    try {
      after(async () => { await overpassPromise; });
    } catch (err) {
      // after() only works inside a request context; if it's ever
      // unavailable the in-flight fetch is simply abandoned when the
      // function returns, same as before this soft-deadline existed --
      // this call still returns null and the report still renders on
      // time, it just loses the "populate the cache anyway" benefit.
      console.error('[noiseScore] after() unavailable, in-flight fetch will be abandoned:', err?.message || err);
    }
    return null;
  }
