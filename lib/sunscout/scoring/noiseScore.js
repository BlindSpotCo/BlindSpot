...
export async function fetchNearbyNoiseSources(lat, lon) {
  ...
  // Raced, not tried one after another -- the worst case for the whole
  // call is now one timeout (~FETCH_TIMEOUT_MS), not the two stacked
  // back-to-back that made a cold address take up to 28s. Promise.any
  // resolves with whichever endpoint answers first and only rejects
  // (AggregateError) if every one of them failed.
  let data = null;
  try {
    data = await Promise.any(OVERPASS_URLS.map((url) => queryOverpass(url, query)));
  } catch {
    data = null;
  }
  if (!data) return null;
  ...
  // rounded query point above -- rounding only widens the net Overpass
  // searches and the cache key, it never blurs this specific property's
  // own numbers.
