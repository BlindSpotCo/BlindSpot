...
import { fetchNearbyNoiseSources, computeNoiseScore, pendingNoiseScore } from './noiseScore';
import { DEFAULT_WEIGHTS, scoreToGrade, clamp } from './types';
import { estimateAcCost } from './thermalCostEstimate';
import { tariffFor, monsoonRainfallFor, monsoonSeasonFor } from '@/lib/aslivastu/cityMeta';
...
export async function computeLiveScore(input) {
  ...
  // The live Overpass (road/rail) call is the one sub-score fetch slow
  // and unpredictable enough (a cold map patch can take several real
  // seconds) that making the whole report wait on it isn't worth it.
  // `skipLiveNoise` lets a caller get everything else back immediately,
  // with a `pending` noise row in its place -- see /api/property-score
  // and /api/sunscout/score, which are called once fast (skipLiveNoise)
  // to render the report right away, then again in the background with
  // the real fetch so the client can patch the real noise score in once
  // it's actually known, rather than never surfacing it at all.
  const skipLiveNoise = input.skipLiveNoise === true;
  ...
  const [solarSummary, windReading, noiseData] = await Promise.all([
    ...
    skipLiveNoise ? Promise.resolve(undefined) : fetchNearbyNoiseSources(lat, lon),
  ]);
  ...
  const dampnessScore = computeDampnessScore(solarSummary, facing, monsoonRainfallFor(city).mm, monsoonSeasonFor(city));
  const noiseScore = skipLiveNoise ? pendingNoiseScore() : computeNoiseScore(noiseData, floor, facing);
  ...
  if (skipLiveNoise) {
    dataNotes.push('Noise score is still checking nearby roads/rail and will update shortly.');
  }
  ...
  return {
    ...
    subScores,
    thermalCost,
    // Month by month usable direct sun for this floor/facing -- already
    // computed for the sun score; the report shows it as a year strip.
    sunYear: (solarSummary.monthlySummary || []).map((m) => ({
      month: m.month, hours: m.usableHours, rise: m.sunrise, set: m.sunset,
    })),
    weights,
    unit: { floor, facing },
    dataNotes,
